...
static void picture_data()
{
	...
	for(;;)
	{
		...
		else if(ret == 0 && d->Fault_Flag){
			#ifdef TRACE
				printf("picture_data() Fault_Flag detected\n");
				ML_log("picture_data() Fault_Flag detected\n");
			#endif
			return;
		}
	}
		...
		unsigned char *r, unsigned char *g, unsigned char *b)
{
    ...
    *r = clipMe((y + crv*v + 32768)>>16);
    *g = clipMe((y - cgu*u - cgv*v + 32768)>>16);
    *b = clipMe((y + cbu*u + 32786)>>16);
}
