...
static void sequence_header()
{
  ...
  if (Verbose_Flag > NO_LAYER)
  {
    ...
    if (Verbose_Flag > SEQUENCE_LAYER)
    {
      ...
      //ML_log("  aspect_ratio_information=%d\n",aspect_ratio_information);
      //ML_log("  frame_rate_code=%d",frame_rate_code);
      //ML_log("  bit_rate_value=%d\n",bit_rate_value);
      //ML_log("  vbv_buffer_size=%d\n",vbv_buffer_size);
      //ML_log("  constrained_parameters_flag=%d\n",constrained_parameters_flag);
...
static void quant_matrix_extension()
{
  ...
  // log it
   ...
   if (Verbose_Flag>NO_LAYER)
  {
    ML_log("quant matrix extension\n");
    ML_log("  load_intra_quantizer_matrix=%d\n",
      ld->load_intra_quantizer_matrix);
    ML_log("  load_non_intra_quantizer_matrix=%d\n",
      ld->load_non_intra_quantizer_matrix);
    ML_log("  load_chroma_intra_quantizer_matrix=%d\n",
      ld->load_chroma_intra_quantizer_matrix);
    ML_log("  load_chroma_non_intra_quantizer_matrix=%d\n",
      ld->load_chroma_non_intra_quantizer_matrix);
  }
...
static void sequence_scalable_extension()
{
  ...
  // log it
   ...
   if (Verbose_Flag>NO_LAYER)
  {
    ML_log("sequence scalable extension\n");
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      ML_log("  scalable_mode=%d\n",ld->scalable_mode-1);
      ML_log("  layer_id=%d\n",layer_id);
      if (ld->scalable_mode==SC_SPAT)
      {
        ML_log("    lower_layer_prediction_horiontal_size=%d\n",
          lower_layer_prediction_horizontal_size);
        ML_log("    lower_layer_prediction_vertical_size=%d\n",
          lower_layer_prediction_vertical_size);
        ML_log("    horizontal_subsampling_factor_m=%d\n",
          horizontal_subsampling_factor_m);
        ML_log("    horizontal_subsampling_factor_n=%d\n",
          horizontal_subsampling_factor_n);
        ML_log("    vertical_subsampling_factor_m=%d\n",
          vertical_subsampling_factor_m);
        ML_log("    vertical_subsampling_factor_n=%d\n",
          vertical_subsampling_factor_n);
      }
    }
  }
...
static void picture_display_extension()
{
  ...
  if (Verbose_Flag>NO_LAYER)
  {
    ML_log("picture display extension\n");
    if (Verbose_Flag>SEQUENCE_LAYER)
    {

      for (i=0; i<number_of_frame_center_offsets; i++)
      {
        ML_log("  frame_center_horizontal_offset[%d]=%d\n",i,
          frame_center_horizontal_offset[i]);
        ML_log("  frame_center_vertical_offset[%d]=%d\n",i,
          frame_center_vertical_offset[i]);
      }
    }
  }
...
static void picture_spatial_scalable_extension()
{
  ...
  // log it
   ...
   if (Verbose_Flag>NO_LAYER)
  {
    ML_log("picture spatial scalable extension\n");
    if (Verbose_Flag>SEQUENCE_LAYER)
    {
      ML_log("  lower_layer_temporal_reference=%d\n",lower_layer_temporal_reference);
      ML_log("  lower_layer_horizontal_offset=%d\n",lower_layer_horizontal_offset);
      ML_log("  lower_layer_vertical_offset=%d\n",lower_layer_vertical_offset);
      ML_log("  spatial_temporal_weight_code_table_index=%d\n",
        spatial_temporal_weight_code_table_index);
      ML_log("  lower_layer_progressive_frame=%d\n",lower_layer_progressive_frame);
      ML_log("  lower_layer_deinterlaced_field_select=%d\n",lower_layer_deinterlaced_field_select);
    }
  }
